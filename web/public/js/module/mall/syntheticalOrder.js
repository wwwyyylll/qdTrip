require(["consts", "apis", "utils", "common"], function(consts, apis, utils) {
    ...
    var lookButton = '<button class="btn btn-info" type="button" data-operate="look">查看详情</button>',
        saveChangeCodeButton =  '<button class="btn btn-success" type="button" data-operate="saveChangeCode">输入兑换码</button>',
        completedButton = '<button class="btn btn-primary" type="button" data-operate="completed">代充完成</button>',
    ...
    var operates = {
        ...
        look:function($this){
            ...
            utils.ajaxSubmit(apis.syntheticalOrder.getById, {id: id}, function (data) {
                var data = {
                    dataArr:data
                };
                data.dataArr.exchangeMethodText = consts.status.exchangeMethod1[data.dataArr.exchangeMethod];
            ...
            utils.renderModal('填写兑换码', template('exchangeModal', getData), function(){
                if($("#exchangeForm").valid()) {
                    hound.confirm('确认兑换吗?', '', function () {
                        utils.loading(true);
                        utils.ajaxSubmit(apis.syntheticalOrder.saveExchangeCodeById, $("#exchangeForm").serialize(), function (data) {
                            utils.loading(false);
        ...
        refund:function($this){
            ...
            hound.reason('确认退款吗?','请输入退款原因',function(data){
                utils.loading(true);
                utils.ajaxSubmit(apis.syntheticalOrder.refundById, {id: id,reason:data}, function (data) {
                    utils.loading(false);
        ...
        completed:function($this){
            var id = $this.closest("tr").attr("data-id");
            hound.confirm('确认代充吗?','',function(data){
                utils.loading(true);
                utils.ajaxSubmit(apis.syntheticalOrder.completedById, {id: id}, function (data) {
                    utils.loading(false);
                    hound.success("操作成功", "", 1000);
                    loadData();
                });
    ...
    function loadData() {
        utils.ajaxSubmit(apis.syntheticalOrder.getLists, param, function (data) {
            //根据状态值显示对应的状态文字
            $.each(data.dataArr,function(i,n){
                ...
                if(n.status=="1"){
                    if(n.exchangeMethod=="1"){
                        n.materialButtonGroup = lookButton + refundButton + saveChangeCodeButton;
                    }else{
                        n.materialButtonGroup = lookButton + refundButton + completedButton;
                    }
                }else{
