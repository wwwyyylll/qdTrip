require(["consts", "apis", "utils", "common"], function(consts, apis, utils) {
    ...
    var operates = {
        ...
        look:function($this){
            ...
            window.open("@@HOSTview/mall/pddOrderDetails.html?id=" + id);
        },
        //完成
        ...
        //同步订单
        syncOrder:function(){
            hound.confirm('确认同步订单吗?', '', function () {
                utils.loading(true);
                utils.ajaxSubmit(apis.pddOrder.syncOrder, '', function (data) {
                    utils.loading(false);
    ...
    function loadData() {
        utils.ajaxSubmit(apis.pddOrder.getLists, param, function (data) {
            $.each(data.dataArr,function(i,n){
                n.statusText = consts.status.pddStatus[n.status];
                n.orderStatusText = consts.status.pddOrderStatus[n.orderStatus];
    ...
    $("#search").on("click",function(){
        ...
        if(selectSearchLabel=="订单号"){
            //订单号搜索
            param.orderSn = $("#searchCont").val();
            param.pId = '';
            loadData();
        }else if(selectSearchLabel=="推广位ID"){
            //商品标题搜索
            param.pId = $("#searchCont").val();
            param.orderSn = '';
            loadData();
        }
    });
