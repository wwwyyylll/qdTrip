require(["consts", "apis", "utils", "common"], function(consts, apis, utils) {
    ...
    $addModal.on("click",function(){
        ...
        utils.renderModal('新增店铺', template('modalDiv',initialData), function(){
            if($("#visaPassportForm").valid()){
                utils.ajaxSubmit(apis.shop.create,$("#visaPassportForm").serialize(),function(data){
    ...
    var operates = {
        ...
        edit:function($this){
            ...
            utils.ajaxSubmit(apis.shop.getById, {id: id}, function (data) {
                var getByIdData = {
                    dataArr:data
                };
                utils.renderModal('编辑店铺', template('modalDiv', getByIdData), function(){
                    if($("#visaPassportForm").valid()) {
                        utils.ajaxSubmit(apis.shop.updateById, $("#visaPassportForm").serialize(), function (data) {
        ...
        look:function($this){
            ...
            utils.ajaxSubmit(apis.shop.getById, {id: id}, function (data) {
                ...
                utils.renderModal('查看店铺', template('modalDiv', getByIdData),'', 'lg');
        ...
        setOff:function($this){
            ...
            hound.confirm('确认无效吗?', '', function () {
                utils.ajaxSubmit(apis.shop.offById, {id: id}, function (data) {
                    loadData();
                });
            });
        },
        ...
        setOn:function($this){
            ...
            hound.confirm('确认有效吗?', '', function () {
                utils.ajaxSubmit(apis.shop.onById, {id: id}, function (data) {
                    loadData();
                });
            });
        }
    ...
    function loadData() {
        utils.ajaxSubmit(apis.shop.getLists, param, function (data) {
            //根据状态值显示对应的状态文字 + 显示对应的 允许登录/禁止登录 按钮
            $.each(data.dataArr,function(i,n){
                n.statusText = consts.status.ordinary[n.status];
                n.sourceText = consts.status.shopSource[n.source];
