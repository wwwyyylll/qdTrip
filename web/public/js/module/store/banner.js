require(["consts", "apis", "utils", "common"], function(consts, apis, utils) {
    ...
    $addModal.on("click",function(){
        ...
        utils.renderModal('新增banner', template('modalDiv',initialData), function(){
            if($("#visaPassportForm").valid()){
                utils.ajaxSubmit(apis.mallBanner.create,$("#visaPassportForm").serialize(),function(data){
    ...
    var operates = {
        ...
        edit:function($this){
            ...
            utils.ajaxSubmit(apis.mallBanner.getById, {id: id}, function (data) {
                var data = {
                    dataArr:data
                };
                utils.renderModal('编辑——'+ data.dataArr.title, template('modalDiv', data), function(){
                    if($("#visaPassportForm").valid()) {
                        utils.ajaxSubmit(apis.mallBanner.updateById, $("#visaPassportForm").serialize(), function (data) {
        ...
        setOff:function($this){
            ...
            hound.confirm('确认无效吗?', '', function () {
                utils.ajaxSubmit(apis.mallBanner.offById, {id: id}, function (data) {
                    loadData();
                });
            });
        },
        ...
        setOn:function($this){
            ...
            hound.confirm('确认有效吗?', '', function () {
                utils.ajaxSubmit(apis.mallBanner.onById, {id: id}, function (data) {
                    loadData();
                });
            });
        }
    ...
    function loadData() {
        utils.ajaxSubmit(apis.mallBanner.getLists, param, function (data) {
            //根据状态值显示对应的状态文字
