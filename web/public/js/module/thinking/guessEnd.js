require(["consts", "apis", "utils", "common"], function(consts, apis, utils) {
        ...
        utils.renderModal('新增', template('modalDiv',initialData), function(){
            ...
            if(itemId!=undefined){
                if($("#visaPassportForm").valid()){
                    utils.ajaxSubmit(apis.guessEnd.create,$("#visaPassportForm").serialize(),function(data){
    ...
    var operates = {
        ...
        addSelect:function($this){
            if($("#itemSelectDiv").find(".singItem").length>=3){
                $this.closest(".form-row").find("h5").text("提示:最多可添加3个问题答案");
            }else{
        ...
        delSelect:function($this){
            if($("#itemSelectDiv").find(".singItem").length<=2){
                $(".mark").closest(".form-row").find("h5").text("提示:最少要添加2个问题答案");
            }else{
        ...
        edit:function($this){
            ...
            utils.ajaxSubmit(apis.guessEnd.getById, {id: id}, function (data) {
                var data = {
                    itemArr:(data.itemArr.length!=0)? data.itemArr : [{},{}],
                    dataArr:data.dataArr
                };
                utils.renderModal('编辑', template('modalDiv', data), function(){
                    utils.reInputName($(".singItem"));
                    utils.reInputName($(".delItemSingle"));
                    if($("#visaPassportForm").valid()) {
                        utils.ajaxSubmit(apis.guessEnd.updateById, $("#visaPassportForm").serialize(), function (data) {
        ...
        look:function($this){
            ...
            utils.ajaxSubmit(apis.guessEnd.getById, {id: id}, function (data) {
                ...
                utils.renderModal('查看', template('modalDiv', data),'', 'lg');
        ...
        settlement:function($this){
            ...
            utils.ajaxSubmit(apis.guessEnd.getById, {id: id}, function (data) {
                ...
                utils.renderModal('结算', template('settlementDiv', data),function(){
                    utils.ajaxSubmit(apis.guessEnd.settlementById,$("#settlementForm").serialize(),function(data){
        ...
        setOff:function($this){
            ...
            hound.confirm('确认无效吗?', '', function () {
                utils.ajaxSubmit(apis.guessEnd.setOff, {id: id}, function (data) {
                    loadData();
                });
            });
        },
        ...
        setOn:function($this){
            ...
            hound.confirm('确认有效吗?', '', function () {
                utils.ajaxSubmit(apis.guessEnd.setOn, {id: id}, function (data) {
                    loadData();
                });
            });
        },
    ...
    function loadData() {
        utils.ajaxSubmit(apis.guessEnd.getLists, param, function (data) {
            //根据状态值显示对应的状态文字
